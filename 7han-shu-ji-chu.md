...
递归函数指的是函数内部又调用了当前的函数，递归调用，与java的递归函数没什么区别吧。
...
尾递归是指，在函数返回的时候，调用自身本身，并且，return语句不能包含表达式。这样，编译器或者解释器就可以把尾递归做优化，使递归本身无论调用多少次，都只占用一个栈帧，不会出现栈溢出的情况。（**然而python解释器并没有优化，所以还是不能防止栈溢出**）
...
* 使用递归函数的优点是**逻辑简单清晰**，缺点是过深的调用会导致**栈溢出**。
